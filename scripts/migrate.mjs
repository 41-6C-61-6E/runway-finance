...
async function runSelfHealingChecks(client) {
  ...
  try {
    ...
    if (accountSensitiveColCheck.rows.length === 0) {
      console.log('[migrate] [self-heal] Adding missing sensitive column to accounts...');
    ...
    if (planFlowsCheck.rows.length > 0) {
      ...
      if (colCheckSalarySource.rows.length === 0) {
        console.log('[migrate] [self-heal] Adding missing salary_source column to plan_flows...');
        await client.query(`
