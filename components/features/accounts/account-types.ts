...
// ── Forecast & Recurring Types ───────────────────────────────────────────────

export type ForecastHorizon = '30d' | '60d' | '90d' | '6m' | '1y';

export type RecurringStreamType = 'income' | 'subscription' | 'bill' | 'loan' | 'transfer' | 'other';
export type RecurringFrequency = 'weekly' | 'biweekly' | 'semimonthly' | 'monthly' | 'quarterly' | 'yearly';

export interface RecurringStreamItem {
  id: string;
  name: string;
  payee: string;
  normalizedName?: string;
  amount: number;
  type: RecurringStreamType;
...
export interface ForecastDayEventData {
  id: string;
  name: string;
  amount: number;
  type: RecurringStreamType | 'one_off';
  accountId?: string | null;
  streamId?: string;
  isCustom?: boolean;
}

export interface BalanceForecastPointData {
  date: string;
  totalBalance: number;
  ...
  outflows: number;
  netDelta: number;
  events: ForecastDayEventData[];
}

export interface HistoricalBalancePointData {
  date: string;
  totalBalance: number;
  accounts: Record<string, number>;
}
