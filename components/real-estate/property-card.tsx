...
export function PropertyCard({ property, onLinkMortgage, onUnlinkMortgage, onOverrideValue, onEditMortgage, onEditProperty }: PropertyCardProps) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 flex flex-col justify-between h-full min-h-[320px]">
      <div>
        {/* Tabs Header */}
        {isPayoffVisible && (
              ...
              <div className="flex items-start gap-2">
                {onEditProperty && (
                  <button
                    ...
                    className="mt-0.5 p-1.5 rounded-lg hover:bg-muted text-muted-foreground/50 hover:text-foreground transition-colors cursor-pointer"
                  >
                  ...
                  {editingValue ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        autoFocus
                      />
                      <button onClick={handleSaveValue} className="text-[10px] text-primary font-medium cursor-pointer">Save</button>
                      <button onClick={() => setEditingValue(false)} className="text-[10px] text-muted-foreground cursor-pointer">Cancel</button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => { setNewValue(String(property.value)); setEditingValue(true); }}
                        className="p-0.5 rounded hover:bg-muted text-muted-foreground/50 hover:text-foreground transition-colors cursor-pointer"
                      >
