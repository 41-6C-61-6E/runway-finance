...
export function RothConversionTab({
  ...
  const buildEnginePlanHelper = (enable: boolean, ceiling: string, irmaaGuard: boolean) => {
    return buildEnginePlan(plan, {
      ...
      rothConversionTargetCeiling: ceiling,
      avoidIrmaaCliffs: irmaaGuard,
    });
  };

  const simNoRoth = useMemo(() => runRetirementSimulation(buildEnginePlanHelper(false, 'top_of_12', false)), [plan]);
  const simActive = useMemo(() => runRetirementSimulation(buildEnginePlanHelper(enableRoth, targetCeiling, avoidIrmaa)), [plan, enableRoth, targetCeiling, avoidIrmaa]);
