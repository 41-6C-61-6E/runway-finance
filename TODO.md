## TO DO LIST

BUGS:
- 

CHANGES:
- Interface flattening
- Move account management section in settings to its own tab


NEW FEATUES:
- Add debt to Asset ratio and asset breakdown graphics to net worth page
- semantic versioning system
- account values over time
- Account summary widget at top of accounts area
- spending summary by month
- cash flow snakey with net income and savings rate
- tabs in the settings area for groupings of settings
- Full transaction management page with filtering, bulk actions, and detail drawer for editing transactions
- All, Spending and Income tabs on the transaciton page with summary charts

FOR THE FRONTIER MODELS:
- manual account support
    - Realestate - API
    - Vehicles - API
    - Bitcoin - API
    - Gold - API
    - Silver - API
    - Other Asset - Manual

- FI number and year-by-year projection using live burn rate data.

- Account forcasting and cash flow, poaaibly with AI integration

- PWA support

- AI Financial fitness evaluation

