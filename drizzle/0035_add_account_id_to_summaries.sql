...
-- 2. Add the account_id column with appropriate foreign key references
ALTER TABLE category_spending_summary ADD COLUMN account_id uuid NOT NULL REFERENCES accounts(id) ON DELETE CASCADE;
ALTER TABLE category_income_summary ADD COLUMN account_id uuid NOT NULL REFERENCES accounts(id) ON DELETE CASCADE;

-- 3. Drop old unique constraints
ALTER TABLE category_spending_summary DROP CONSTRAINT category_spending_summary_user_id_category_id_year_month_unique;
ALTER TABLE category_income_summary DROP CONSTRAINT unique_user_category_month_income;
