...
export const paystubs = pgTable('paystubs', {
  ...
  // P-2 (TAX_PAYROLL_REVIEW.md): true when the imported stub ties out —
  // |gross − taxes − deductions − net| < $0.02 and (when the employer
  // supplies YTD figures) the YTD deltas are continuous vs the prior
  // imported stub. NULL for old data / auto-generated stubs.
  tiesOut: boolean('ties_out'),
  // P-4: stable dedupe key for employers without advice numbers:
  // hash(userId, employer, checkDate, gross, taxes). NULL when an
  // adviceNumber covers dedupe.
